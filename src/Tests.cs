...
module IptablesToSmt {
    ...
    decreases args
  {
    ...
    if exists k: int {:trigger payload[k]} :: 0 <= k < |payload| && payload[k] == '\r' {
    ...
    decreases lines
  {
      ...
      decreases |lines| - idx
    {
      ...
      if |trimmed| >= 2 && trimmed[0 .. 2] == ""-A"" {
        ...
        if parsedOk {
          collected := collected + [parsedRule];
        }
      }
      idx := idx + 1;
    }
  ...
  function FindMatch(tokens: seq<string>, flag: string): FieldMatch
    requires forall t: seq<char> {:trigger |t|} {:trigger t in tokens} :: t in tokens ==> |t| > 0
    ensures FindMatch(tokens, flag).MatchExact? ==> |FindMatch(tokens, flag).value| > 0
    decreases tokens, flag
  {
    if |tokens| < 2 then
      MatchAny
    else if tokens[0] == flag then
      MatchExact(tokens[1])
    else
      FindMatch(tokens[1..], flag)
  }
  ...
  method ParseRuleTokens(tokens: seq<string>, lineNumber: int, rawLine: string)
    ...
    decreases tokens, lineNumber, rawLine
  {
    if |tokens| < 3 {
    ...
    decreases if n < 0 then -n else n
  {
    if n == 0 then
      ...
      var value: int := if n < 0 then -n else n; var digits: string := DigitsToString(value); if n < 0 then ""-"" + digits else digits
  }
    ...
    decreases n
  {
    var digit: int := n % 10;
    var rest: int := n / 10;
    var dStr: string := DigitToString(digit);
    if rest == 0 then
      dStr
    else
      DigitsToString(rest) + dStr
  }
  ...
  function BuildRuleConditions(rule: Rule): seq<string>
    decreases rule
  {
    var chainLiteral: string := FormatStringLiteral(rule.chain);
    var c1: seq<seq<char>> := [""(= pkt_chain "" + chainLiteral + "")""];
    var c2: seq<string> := MaybeAppend(c1, ""pkt_src"", rule.source);
    var c3: seq<string> := MaybeAppend(c2, ""pkt_dst"", rule.destination);
    var c4: seq<string> := MaybeAppend(c3, ""pkt_proto"", rule.protocol);
    var c5: seq<string> := MaybeAppend(c4, ""pkt_sport"", rule.srcPort);
    var c6: seq<string> := MaybeAppend(c5, ""pkt_dport"", rule.dstPort);
    c6
  }

  function MaybeAppend(conds: seq<string>, fieldName: string, crit: FieldMatch): seq<string>
    decreases conds, fieldName, crit
  {
    match crit
    case MatchExact(value) =>
      var literal: string := FormatStringLiteral(value);
      var fragment: seq<char> := ""(= "" + fieldName + "" "" + literal + "")"";
      conds + [fragment]
    case MatchAny() =>
      conds
  }
    ...
    decreases |text|
  {
    if |text| == 0 then
      ...
      var ch: char := text[0]; var rest: string := EscapeForSmtRecursive(text[1..]); if ch == '""' || ch == '\\' then ""\\"" + text[0 .. 1] + rest else text[0 .. 1] + rest
  }

  function FormatRule(rule: Rule, index: int): string
    requires index >= 0
    decreases rule, index
  {
    var indexText: string := IntToString(index);
    var lineText: string := IntToString(rule.lineNumber);
    var header: seq<char> := ""; Rule "" + indexText + "" (line "" + lineText + ""): "" + rule.original + ""\n"";
    var def: seq<char> := ""(define-fun rule"" + indexText + "" ((pkt_chain String) (pkt_src String) (pkt_dst String) (pkt_proto String) (pkt_sport String) (pkt_dport String)) Bool\n"";
    var conditions: seq<string> := BuildRuleConditions(rule);
    var condBuilder: seq<char> := if |conditions| == 0 then ""  true\n"" else ""  (and\n"" + JoinLinesIndent(conditions) + ""  )\n"";
    var closing: string := "")\n"";
    var targetLiteral: string := FormatTargetLiteral(rule.target);
    var action: seq<char> := ""(assert (=> (rule"" + indexText + "" packet_chain packet_src packet_dst packet_proto packet_sport packet_dport)\n"" + ""            (= packet_action "" + targetLiteral + "")))\n"";
  ...
  {
    |s1| == |s2| &&
    forall i: int {:trigger s2[i]} {:trigger s1[i]} :: 
      0 <= i < |s1| ==>
        ToUp(s1[i]) == ToUp(s2[i])
  }

  predicate MatchesTarget(target: Target, raw: string)
    decreases target, raw
  {
    match target
    case TargetAccept() =>
    ...
    decreases left, right
  {
    ...
    while i < |left|
      invariant 0 <= i <= |left|
      invariant forall k: int {:trigger right[k]} {:trigger left[k]} :: 0 <= k < i ==> ToUp(left[k]) == ToUp(right[k])
      decreases |left| - i
    {
    ...
    decreases parts, delim
  {
    if |parts| == 0 then
      ...
      parts[0] + delim + Join(parts[1..], delim)
  }

  lemma /*{:_inductionTrigger Join(parts, delim) + delim}*/ /*{:_inductionTrigger Join(parts + [suffix], delim)}*/ /*{:_induction parts, delim}*/ JoinDistributes(parts: seq<string>, delim: string, suffix: string)
    ensures |parts| > 0 ==> Join(parts + [suffix], delim) == Join(parts, delim) + delim + suffix
    ensures |parts| == 0 ==> Join(parts + [suffix], delim) == suffix
    decreases parts, delim, suffix
  {
    if |parts| == 0 {
    } else {
      if |parts| == 1 {
        ...
        ghost var p0 := parts[0];
        ghost var rest := parts[1..];
  ...
  method SplitLines(text: string) returns (lines: seq<string>)
    requires forall k: int {:trigger text[k]} :: 0 <= k < |text| ==> text[k] != '\r'
    ensures Join(lines, ""\n"") == text
    decreases text
  {
    if |text| == 0 {
  ...
  method SplitOnWhitespace(text: string) returns (tokens: seq<string>)
    ensures forall t: seq<char> {:trigger |t|} {:trigger t in tokens} :: t in tokens ==> |t| <= |text|
    ensures forall t: seq<char> {:trigger |t|} {:trigger t in tokens} :: t in tokens ==> |t| > 0
    decreases text
  {
    ...
    while i < |text|
      invariant 0 <= i
      invariant i <= |text|
      invariant forall t: seq<char> {:trigger |t|} {:trigger t in parts} :: t in parts ==> |t| <= |text|
      invariant forall t: seq<char> {:trigger |t|} {:trigger t in parts} :: t in parts ==> |t| > 0
      decreases |text| - i
    {
    ...
    decreases text
  {
    ...
    while start < stop && IsWhitespace(text[start])
      ...
      invariant stop <= |text|
      invariant forall k: int {:trigger text[k]} :: 0 <= k < start ==> IsWhitespace(text[k])
    ...
    while stop > start && IsWhitespace(text[stop - 1])
      ...
      invariant stop <= |text|
      invariant forall k: int {:trigger text[k]} :: 0 <= k < start ==> IsWhitespace(text[k])
      invariant forall k: int {:trigger text[k]} :: stop <= k < |text| ==> IsWhitespace(text[k])
      decreases stop - start
    {
      stop := stop - 1;
    }
  ...
  datatype Target = TargetAccept | TargetDrop | TargetReject | TargetReturn | TargetJump(name: string)

  datatype FieldMatch = MatchAny | MatchExact(value: string)

  datatype Rule = Rule(chain: string, source: FieldMatch, destination: FieldMatch, protocol: FieldMatch, srcPort: FieldMatch, dstPort: FieldMatch, target: Target, lineNumber: int, original: string)
}
")]
...
namespace IptablesToSmt {
  ...
  public partial class __default {
    public static void _Main(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> args)
    {
      ...
      Dafny.ISequence<Dafny.Rune> _0_payload;
      _0_payload = (args).Select((new BigInteger((args).Count)) - (BigInteger.One));
      if (Dafny.Helpers.Id<Func<Dafny.ISequence<Dafny.Rune>, bool>>((_1_payload) => Dafny.Helpers.Quantifier<BigInteger>(Dafny.Helpers.IntegerRange(BigInteger.Zero, new BigInteger((_1_payload).Count)), false, (((_exists_var_0) => {
        BigInteger _2_k = (BigInteger)_exists_var_0;
        return (((_2_k).Sign != -1) && ((_2_k) < (new BigInteger((_1_payload).Count)))) && (((_1_payload).Select(_2_k)) == (new Dafny.Rune('\r')));
      }))))(_0_payload)) {
        Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Error: Input contains carriage returns (\\r). Please normalize to \\n only.\n")).ToVerbatimString(false));
        return ;
      }
      Dafny.ISequence<Dafny.Rune> _3_smt;
      Dafny.ISequence<Dafny.Rune> _out0;
      _out0 = IptablesToSmt.__default.ConvertIptablesToSmt(_0_payload);
      _3_smt = _out0;
      Dafny.Helpers.Print((_3_smt).ToVerbatimString(false));
    }
    public static void PrintUsage()
    {
      Dafny.ISequence<Dafny.Rune> _0_usage;
      Dafny.ISequence<Dafny.Rune> _out0;
      _out0 = IptablesToSmt.__default.GetUsageString();
      _0_usage = _out0;
      Dafny.Helpers.Print((_0_usage).ToVerbatimString(false));
    }
    public static Dafny.ISequence<Dafny.Rune> GetUsageString()
    {
      Dafny.ISequence<Dafny.Rune> text = Dafny.Sequence<Dafny.Rune>.Empty;
      text = Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Usage:\n  dafny run src/IptablesToSmt.dfy -- \"$(cat rules.txt)\"\n  ./scripts/iptables_to_smt.sh rules.txt\n");
      return text;
    }
    public static Dafny.ISequence<Dafny.Rune> ConvertIptablesToSmt(Dafny.ISequence<Dafny.Rune> input)
    {
      Dafny.ISequence<Dafny.Rune> smt = Dafny.Sequence<Dafny.Rune>.Empty;
      Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _0_lines;
      Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _out0;
      _out0 = IptablesToSmt.__default.SplitLines(input);
      _0_lines = _out0;
      Dafny.ISequence<IptablesToSmt._IRule> _1_rules;
      Dafny.ISequence<IptablesToSmt._IRule> _out1;
      _out1 = IptablesToSmt.__default.ParseRules(_0_lines);
      _1_rules = _out1;
      Dafny.ISequence<Dafny.Rune> _out2;
      _out2 = IptablesToSmt.__default.BuildSmtDocument(_1_rules);
      smt = _out2;
      return smt;
    }
    public static Dafny.ISequence<IptablesToSmt._IRule> ParseRules(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> lines)
    {
      Dafny.ISequence<IptablesToSmt._IRule> rules = Dafny.Sequence<IptablesToSmt._IRule>.Empty;
      Dafny.ISequence<IptablesToSmt._IRule> _0_collected;
      _0_collected = Dafny.Sequence<IptablesToSmt._IRule>.FromElements();
      BigInteger _1_idx;
      _1_idx = BigInteger.Zero;
      while ((_1_idx) < (new BigInteger((lines).Count))) {
        Dafny.ISequence<Dafny.Rune> _2_rawLine;
        _2_rawLine = (lines).Select(_1_idx);
        Dafny.ISequence<Dafny.Rune> _3_trimmed;
        Dafny.ISequence<Dafny.Rune> _out0;
        _out0 = IptablesToSmt.__default.Trim(_2_rawLine);
        _3_trimmed = _out0;
        if (((new BigInteger((_3_trimmed).Count)).Sign == 1) && (((_3_trimmed).Select(BigInteger.Zero)) == (new Dafny.Rune('#')))) {
          _1_idx = (_1_idx) + (BigInteger.One);
          goto continue_0;
        }
        if (((new BigInteger((_3_trimmed).Count)) >= (new BigInteger(2))) && (((_3_trimmed).Subsequence(BigInteger.Zero, new BigInteger(2))).Equals(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("-A")))) {
          Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _4_tokens;
          Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _out1;
          _out1 = IptablesToSmt.__default.SplitOnWhitespace(_3_trimmed);
          _4_tokens = _out1;
          IptablesToSmt._IRule _5_parsedRule;
          bool _6_parsedOk;
          IptablesToSmt._IRule _out2;
          bool _out3;
          IptablesToSmt.__default.ParseRuleTokens(_4_tokens, (_1_idx) + (BigInteger.One), _2_rawLine, out _out2, out _out3);
          _5_parsedRule = _out2;
          _6_parsedOk = _out3;
          if (_6_parsedOk) {
            _0_collected = Dafny.Sequence<IptablesToSmt._IRule>.Concat(_0_collected, Dafny.Sequence<IptablesToSmt._IRule>.FromElements(_5_parsedRule));
          }
        }
        _1_idx = (_1_idx) + (BigInteger.One);
      continue_0: ;
    ...
    public static IptablesToSmt._IFieldMatch FindMatch(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> tokens, Dafny.ISequence<Dafny.Rune> flag)
    {
    TAIL_CALL_START: ;
      if ((new BigInteger((tokens).Count)) < (new BigInteger(2))) {
        return IptablesToSmt.FieldMatch.create_MatchAny();
      } else if (((tokens).Select(BigInteger.Zero)).Equals(flag)) {
        return IptablesToSmt.FieldMatch.create_MatchExact((tokens).Select(BigInteger.One));
      } else {
        Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _in0 = (tokens).Drop(BigInteger.One);
        Dafny.ISequence<Dafny.Rune> _in1 = flag;
        tokens = _in0;
        flag = _in1;
        goto TAIL_CALL_START;
      }
    }
    public static bool ValidTokens(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> tokens) {
    TAIL_CALL_START: ;
      if ((new BigInteger((tokens).Count)) < (BigInteger.One)) {
        return true;
      } else {
    ...
    public static void ParseRuleTokens(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> tokens, BigInteger lineNumber, Dafny.ISequence<Dafny.Rune> rawLine, out IptablesToSmt._IRule rule, out bool ok)
    {
      rule = IptablesToSmt.Rule.Default();
      ok = false;
      if ((new BigInteger((tokens).Count)) < (new BigInteger(3))) {
        rule = IptablesToSmt.Rule.create(Dafny.Sequence<Dafny.Rune>.UnicodeFromString(""), IptablesToSmt.FieldMatch.create_MatchAny(), IptablesToSmt.FieldMatch.create_MatchAny(), IptablesToSmt.FieldMatch.create_MatchAny(), IptablesToSmt.FieldMatch.create_MatchAny(), IptablesToSmt.FieldMatch.create_MatchAny(), IptablesToSmt.Target.create_TargetReturn(), lineNumber, rawLine);
        ok = false;
        return ;
      }
      Dafny.ISequence<Dafny.Rune> _0_chain;
      _0_chain = (tokens).Select(BigInteger.One);
      Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _1_rest;
      _1_rest = (tokens).Drop(new BigInteger(2));
      IptablesToSmt._IFieldMatch _2_src;
      _2_src = IptablesToSmt.__default.FindMatch(_1_rest, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("-s"));
      IptablesToSmt._IFieldMatch _3_dst;
      _3_dst = IptablesToSmt.__default.FindMatch(_1_rest, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("-d"));
      IptablesToSmt._IFieldMatch _4_proto;
      _4_proto = IptablesToSmt.__default.FindMatch(_1_rest, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("-p"));
      IptablesToSmt._IFieldMatch _5_srcPort;
      _5_srcPort = IptablesToSmt.__default.FindMatch(_1_rest, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("--sport"));
      IptablesToSmt._IFieldMatch _6_dstPort;
      _6_dstPort = IptablesToSmt.__default.FindMatch(_1_rest, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("--dport"));
      IptablesToSmt._IFieldMatch _7_targetMatch;
      _7_targetMatch = IptablesToSmt.__default.FindMatch(_1_rest, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("-j"));
      IptablesToSmt._ITarget _8_target = IptablesToSmt.Target.Default();
      bool _9_targetOk;
      _9_targetOk = false;
      if ((_7_targetMatch).is_MatchExact) {
        IptablesToSmt._ITarget _out0;
        _out0 = IptablesToSmt.__default.ParseTarget((_7_targetMatch).dtor_value);
        _8_target = _out0;
        _9_targetOk = true;
      } else {
        _8_target = IptablesToSmt.Target.create_TargetReturn();
      }
      bool _10_validFlags;
      _10_validFlags = IptablesToSmt.__default.ValidTokens(_1_rest);
      if ((_10_validFlags) && (_9_targetOk)) {
        rule = IptablesToSmt.Rule.create(_0_chain, _2_src, _3_dst, _4_proto, _5_srcPort, _6_dstPort, _8_target, lineNumber, rawLine);
        ok = true;
      } else {
        rule = IptablesToSmt.Rule.create(_0_chain, _2_src, _3_dst, _4_proto, _5_srcPort, _6_dstPort, _8_target, lineNumber, rawLine);
        ok = false;
    ...
    public static Dafny.ISequence<Dafny.Rune> BuildSmtDocument(Dafny.ISequence<IptablesToSmt._IRule> rules)
    {
      ...
      _0_builder = Dafny.Sequence<Dafny.Rune>.Concat(_0_builder, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(declare-const packet_src String)\n"));
      _0_builder = Dafny.Sequence<Dafny.Rune>.Concat(_0_builder, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(declare-const packet_dst String)\n"));
      _0_builder = Dafny.Sequence<Dafny.Rune>.Concat(_0_builder, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(declare-const packet_proto String)\n"));
      _0_builder = Dafny.Sequence<Dafny.Rune>.Concat(_0_builder, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(declare-const packet_sport String)\n"));
      _0_builder = Dafny.Sequence<Dafny.Rune>.Concat(_0_builder, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(declare-const packet_dport String)\n"));
      ...
      while ((_1_i) < (new BigInteger((rules).Count))) {
        Dafny.ISequence<Dafny.Rune> _2_formatted;
        _2_formatted = IptablesToSmt.__default.FormatRule((rules).Select(_1_i), _1_i);
        _0_builder = Dafny.Sequence<Dafny.Rune>.Concat(_0_builder, _2_formatted);
        _0_builder = Dafny.Sequence<Dafny.Rune>.Concat(_0_builder, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n"));
    ...
    public static Dafny.ISequence<Dafny.Rune> IntToString(BigInteger n) {
      if ((n).Sign == 0) {
        ...
        BigInteger _0_value = (((n).Sign == -1) ? ((BigInteger.Zero) - (n)) : (n));
        Dafny.ISequence<Dafny.Rune> _1_digits = IptablesToSmt.__default.DigitsToString(_0_value);
        if ((n).Sign == -1) {
          return Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("-"), _1_digits);
        } else {
          return _1_digits;
        }
      }
    }
    public static Dafny.ISequence<Dafny.Rune> DigitsToString(BigInteger n) {
      Dafny.ISequence<Dafny.Rune> _0___accumulator = Dafny.Sequence<Dafny.Rune>.FromElements();
    TAIL_CALL_START: ;
      BigInteger _1_digit = Dafny.Helpers.EuclideanModulus(n, new BigInteger(10));
      BigInteger _2_rest = Dafny.Helpers.EuclideanDivision(n, new BigInteger(10));
      Dafny.ISequence<Dafny.Rune> _3_dStr = IptablesToSmt.__default.DigitToString(_1_digit);
      if ((_2_rest).Sign == 0) {
        return Dafny.Sequence<Dafny.Rune>.Concat(_3_dStr, _0___accumulator);
      } else {
        _0___accumulator = Dafny.Sequence<Dafny.Rune>.Concat(_3_dStr, _0___accumulator);
        BigInteger _in0 = _2_rest;
        n = _in0;
        goto TAIL_CALL_START;
      }
    }
    public static Dafny.ISequence<Dafny.Rune> DigitToString(BigInteger d) {
      return (Dafny.Sequence<Dafny.Rune>.UnicodeFromString("0123456789")).Subsequence(d, (d) + (BigInteger.One));
    }
    public static Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> BuildRuleConditions(IptablesToSmt._IRule rule) {
      Dafny.ISequence<Dafny.Rune> _0_chainLiteral = IptablesToSmt.__default.FormatStringLiteral((rule).dtor_chain);
      Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _1_c1 = Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.FromElements(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(= pkt_chain "), _0_chainLiteral), Dafny.Sequence<Dafny.Rune>.UnicodeFromString(")")));
      Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _2_c2 = IptablesToSmt.__default.MaybeAppend(_1_c1, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("pkt_src"), (rule).dtor_source);
      Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _3_c3 = IptablesToSmt.__default.MaybeAppend(_2_c2, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("pkt_dst"), (rule).dtor_destination);
      Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _4_c4 = IptablesToSmt.__default.MaybeAppend(_3_c3, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("pkt_proto"), (rule).dtor_protocol);
      Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _5_c5 = IptablesToSmt.__default.MaybeAppend(_4_c4, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("pkt_sport"), (rule).dtor_srcPort);
      Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _6_c6 = IptablesToSmt.__default.MaybeAppend(_5_c5, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("pkt_dport"), (rule).dtor_dstPort);
      return _6_c6;
    }
    public static Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> MaybeAppend(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> conds, Dafny.ISequence<Dafny.Rune> fieldName, IptablesToSmt._IFieldMatch crit)
    {
      IptablesToSmt._IFieldMatch _source0 = crit;
      {
        if (_source0.is_MatchExact) {
          Dafny.ISequence<Dafny.Rune> _0_value = _source0.dtor_value;
          Dafny.ISequence<Dafny.Rune> _1_literal = IptablesToSmt.__default.FormatStringLiteral(_0_value);
          Dafny.ISequence<Dafny.Rune> _2_fragment = Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(= "), fieldName), Dafny.Sequence<Dafny.Rune>.UnicodeFromString(" ")), _1_literal), Dafny.Sequence<Dafny.Rune>.UnicodeFromString(")"));
          return Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.Concat(conds, Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.FromElements(_2_fragment));
        }
      }
      {
        return conds;
    ...
    public static Dafny.ISequence<Dafny.Rune> FormatTargetLiteral(IptablesToSmt._ITarget target) {
      return IptablesToSmt.__default.FormatStringLiteral(IptablesToSmt.__default.TargetToString(target));
    }
    public static Dafny.ISequence<Dafny.Rune> TargetToString(IptablesToSmt._ITarget target) {
      IptablesToSmt._ITarget _source0 = target;
      {
        if (_source0.is_TargetAccept) {
          return Dafny.Sequence<Dafny.Rune>.UnicodeFromString("ACCEPT");
        }
      }
      {
        if (_source0.is_TargetDrop) {
          return Dafny.Sequence<Dafny.Rune>.UnicodeFromString("DROP");
        }
      }
      {
        if (_source0.is_TargetReject) {
          return Dafny.Sequence<Dafny.Rune>.UnicodeFromString("REJECT");
        }
      }
      {
        if (_source0.is_TargetReturn) {
          return Dafny.Sequence<Dafny.Rune>.UnicodeFromString("RETURN");
        }
      }
      {
    ...
    public static Dafny.ISequence<Dafny.Rune> FormatStringLiteral(Dafny.ISequence<Dafny.Rune> text) {
      return Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\""), IptablesToSmt.__default.EscapeForSmt(text)), Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\""));
    }
    public static Dafny.ISequence<Dafny.Rune> EscapeForSmt(Dafny.ISequence<Dafny.Rune> text) {
      return IptablesToSmt.__default.EscapeForSmtRecursive(text);
    }
    public static Dafny.ISequence<Dafny.Rune> EscapeForSmtRecursive(Dafny.ISequence<Dafny.Rune> text) {
      if ((new BigInteger((text).Count)).Sign == 0) {
        return Dafny.Sequence<Dafny.Rune>.UnicodeFromString("");
      } else {
        Dafny.Rune _0_ch = (text).Select(BigInteger.Zero);
        Dafny.ISequence<Dafny.Rune> _1_rest = IptablesToSmt.__default.EscapeForSmtRecursive((text).Drop(BigInteger.One));
        if (((_0_ch) == (new Dafny.Rune('"'))) || ((_0_ch) == (new Dafny.Rune('\\')))) {
          return Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\\"), (text).Subsequence(BigInteger.Zero, BigInteger.One)), _1_rest);
        } else {
          return Dafny.Sequence<Dafny.Rune>.Concat((text).Subsequence(BigInteger.Zero, BigInteger.One), _1_rest);
        }
      }
    }
    public static Dafny.ISequence<Dafny.Rune> FormatRule(IptablesToSmt._IRule rule, BigInteger index)
    {
      Dafny.ISequence<Dafny.Rune> _0_indexText = IptablesToSmt.__default.IntToString(index);
      Dafny.ISequence<Dafny.Rune> _1_lineText = IptablesToSmt.__default.IntToString((rule).dtor_lineNumber);
      Dafny.ISequence<Dafny.Rune> _2_header = Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("; Rule "), _0_indexText), Dafny.Sequence<Dafny.Rune>.UnicodeFromString(" (line ")), _1_lineText), Dafny.Sequence<Dafny.Rune>.UnicodeFromString("): ")), (rule).dtor_original), Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n"));
      Dafny.ISequence<Dafny.Rune> _3_def = Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(define-fun rule"), _0_indexText), Dafny.Sequence<Dafny.Rune>.UnicodeFromString(" ((pkt_chain String) (pkt_src String) (pkt_dst String) (pkt_proto String) (pkt_sport String) (pkt_dport String)) Bool\n"));
      Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _4_conditions = IptablesToSmt.__default.BuildRuleConditions(rule);
      Dafny.ISequence<Dafny.Rune> _5_condBuilder = (((new BigInteger((_4_conditions).Count)).Sign == 0) ? (Dafny.Sequence<Dafny.Rune>.UnicodeFromString("  true\n")) : (Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("  (and\n"), IptablesToSmt.__default.JoinLinesIndent(_4_conditions)), Dafny.Sequence<Dafny.Rune>.UnicodeFromString("  )\n"))));
      Dafny.ISequence<Dafny.Rune> _6_closing = Dafny.Sequence<Dafny.Rune>.UnicodeFromString(")\n");
      Dafny.ISequence<Dafny.Rune> _7_targetLiteral = IptablesToSmt.__default.FormatTargetLiteral((rule).dtor_target);
      Dafny.ISequence<Dafny.Rune> _8_action = Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(assert (=> (rule"), _0_indexText), Dafny.Sequence<Dafny.Rune>.UnicodeFromString(" packet_chain packet_src packet_dst packet_proto packet_sport packet_dport)\n")), Dafny.Sequence<Dafny.Rune>.UnicodeFromString("            (= packet_action ")), _7_targetLiteral), Dafny.Sequence<Dafny.Rune>.UnicodeFromString(")))\n"));
      return Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(_2_header, _3_def), _5_condBuilder), _6_closing), _8_action);
    }
    public static Dafny.ISequence<Dafny.Rune> JoinLinesIndent(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> lines) {
      Dafny.ISequence<Dafny.Rune> _0___accumulator = Dafny.Sequence<Dafny.Rune>.FromElements();
    TAIL_CALL_START: ;
      if ((new BigInteger((lines).Count)).Sign == 0) {
        return Dafny.Sequence<Dafny.Rune>.Concat(_0___accumulator, Dafny.Sequence<Dafny.Rune>.UnicodeFromString(""));
      } else {
        _0___accumulator = Dafny.Sequence<Dafny.Rune>.Concat(_0___accumulator, Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.Concat(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("    "), (lines).Select(BigInteger.Zero)), Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")));
        Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _in0 = (lines).Drop(BigInteger.One);
        lines = _in0;
        goto TAIL_CALL_START;
      }
    }
    public static Dafny.Rune ToUp(Dafny.Rune ch) {
      if (((new Dafny.Rune('a')) <= (ch)) && ((ch) <= (new Dafny.Rune('z')))) {
        return new Dafny.Rune((int)(((new BigInteger((ch).Value)) - (new BigInteger((new Dafny.Rune('a')).Value))) + (new BigInteger((new Dafny.Rune('A')).Value))));
      } else {
        return ch;
      }
    }
    public static bool EqualsIgnoreCase(Dafny.ISequence<Dafny.Rune> s1, Dafny.ISequence<Dafny.Rune> s2)
    {
      return ((new BigInteger((s1).Count)) == (new BigInteger((s2).Count))) && (Dafny.Helpers.Id<Func<Dafny.ISequence<Dafny.Rune>, Dafny.ISequence<Dafny.Rune>, bool>>((_0_s1, _1_s2) => Dafny.Helpers.Quantifier<BigInteger>(Dafny.Helpers.IntegerRange(BigInteger.Zero, new BigInteger((_0_s1).Count)), true, (((_forall_var_0) => {
        BigInteger _2_i = (BigInteger)_forall_var_0;
        return !(((_2_i).Sign != -1) && ((_2_i) < (new BigInteger((_0_s1).Count)))) || ((IptablesToSmt.__default.ToUp((_0_s1).Select(_2_i))) == (IptablesToSmt.__default.ToUp((_1_s2).Select(_2_i))));
      }))))(s1, s2));
    }
    public static bool MatchesTarget(IptablesToSmt._ITarget target, Dafny.ISequence<Dafny.Rune> raw)
    {
      IptablesToSmt._ITarget _source0 = target;
      {
        if (_source0.is_TargetAccept) {
          return IptablesToSmt.__default.EqualsIgnoreCase(raw, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("ACCEPT"));
        }
      }
      {
        if (_source0.is_TargetDrop) {
          return IptablesToSmt.__default.EqualsIgnoreCase(raw, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("DROP"));
        }
      }
      {
        if (_source0.is_TargetReject) {
          return IptablesToSmt.__default.EqualsIgnoreCase(raw, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("REJECT"));
        }
      }
      {
        if (_source0.is_TargetReturn) {
          return IptablesToSmt.__default.EqualsIgnoreCase(raw, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("RETURN"));
        }
      }
      {
        Dafny.ISequence<Dafny.Rune> _0_name = _source0.dtor_name;
        return (((((_0_name).Equals(raw)) && (!(IptablesToSmt.__default.EqualsIgnoreCase(raw, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("ACCEPT"))))) && (!(IptablesToSmt.__default.EqualsIgnoreCase(raw, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("DROP"))))) && (!(IptablesToSmt.__default.EqualsIgnoreCase(raw, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("REJECT"))))) && (!(IptablesToSmt.__default.EqualsIgnoreCase(raw, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("RETURN"))));
      }
    }
    public static IptablesToSmt._ITarget ParseTarget(Dafny.ISequence<Dafny.Rune> raw)
    {
      IptablesToSmt._ITarget target = IptablesToSmt.Target.Default();
      bool _0_isAccept;
      bool _out0;
      _out0 = IptablesToSmt.__default.StringsEqualIgnoreCase(raw, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("ACCEPT"));
      _0_isAccept = _out0;
      if (_0_isAccept) {
        target = IptablesToSmt.Target.create_TargetAccept();
        return target;
      }
      bool _1_isDrop;
      bool _out1;
      _out1 = IptablesToSmt.__default.StringsEqualIgnoreCase(raw, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("DROP"));
      _1_isDrop = _out1;
      if (_1_isDrop) {
        target = IptablesToSmt.Target.create_TargetDrop();
        return target;
      }
      bool _2_isReject;
      bool _out2;
      _out2 = IptablesToSmt.__default.StringsEqualIgnoreCase(raw, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("REJECT"));
      _2_isReject = _out2;
      if (_2_isReject) {
        target = IptablesToSmt.Target.create_TargetReject();
        return target;
      }
      bool _3_isReturn;
      bool _out3;
      _out3 = IptablesToSmt.__default.StringsEqualIgnoreCase(raw, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("RETURN"));
      _3_isReturn = _out3;
      if (_3_isReturn) {
        target = IptablesToSmt.Target.create_TargetReturn();
        return target;
      }
      target = IptablesToSmt.Target.create_TargetJump(raw);
      return target;
    }
    public static bool StringsEqualIgnoreCase(Dafny.ISequence<Dafny.Rune> left, Dafny.ISequence<Dafny.Rune> right)
    {
      bool eq = false;
      if ((new BigInteger((left).Count)) != (new BigInteger((right).Count))) {
        eq = false;
        return eq;
      }
      BigInteger _0_i;
      ...
      eq = true;
      while ((_0_i) < (new BigInteger((left).Count))) {
        Dafny.Rune _1_leftUp;
        Dafny.Rune _out0;
        _out0 = IptablesToSmt.__default.ToUpperChar((left).Select(_0_i));
        _1_leftUp = _out0;
        Dafny.Rune _2_rightUp;
        Dafny.Rune _out1;
        _out1 = IptablesToSmt.__default.ToUpperChar((right).Select(_0_i));
        _2_rightUp = _out1;
        if ((_1_leftUp) != (_2_rightUp)) {
          eq = false;
          return eq;
        }
    ...
    public static Dafny.Rune ToUpperChar(Dafny.Rune ch)
    {
      Dafny.Rune upper = new Dafny.Rune('D');
      if (((new Dafny.Rune('a')) <= (ch)) && ((ch) <= (new Dafny.Rune('z')))) {
        upper = new Dafny.Rune((int)(((new BigInteger((ch).Value)) - (new BigInteger((new Dafny.Rune('a')).Value))) + (new BigInteger((new Dafny.Rune('A')).Value))));
      } else {
        upper = ch;
      }
      return upper;
    }
    public static Dafny.ISequence<Dafny.Rune> Join(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> parts, Dafny.ISequence<Dafny.Rune> delim)
    {
      Dafny.ISequence<Dafny.Rune> _0___accumulator = Dafny.Sequence<Dafny.Rune>.FromElements();
    TAIL_CALL_START: ;
      if ((new BigInteger((parts).Count)).Sign == 0) {
        return Dafny.Sequence<Dafny.Rune>.Concat(_0___accumulator, Dafny.Sequence<Dafny.Rune>.UnicodeFromString(""));
      } else if ((new BigInteger((parts).Count)) == (BigInteger.One)) {
        return Dafny.Sequence<Dafny.Rune>.Concat(_0___accumulator, (parts).Select(BigInteger.Zero));
      } else {
        _0___accumulator = Dafny.Sequence<Dafny.Rune>.Concat(_0___accumulator, Dafny.Sequence<Dafny.Rune>.Concat((parts).Select(BigInteger.Zero), delim));
        Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _in0 = (parts).Drop(BigInteger.One);
        Dafny.ISequence<Dafny.Rune> _in1 = delim;
        parts = _in0;
        delim = _in1;
        goto TAIL_CALL_START;
      }
    }
    public static Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> SplitLines(Dafny.ISequence<Dafny.Rune> text)
    {
      Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> lines = Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.Empty;
      if ((new BigInteger((text).Count)).Sign == 0) {
        lines = Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.FromElements(Dafny.Sequence<Dafny.Rune>.UnicodeFromString(""));
        return lines;
      }
      Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _0_segments;
      _0_segments = Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.FromElements();
      BigInteger _1_start;
      _1_start = BigInteger.Zero;
      BigInteger _2_i;
      _2_i = BigInteger.Zero;
      while ((_2_i) < (new BigInteger((text).Count))) {
        Dafny.Rune _3_ch;
        _3_ch = (text).Select(_2_i);
        if ((_3_ch) == (new Dafny.Rune('\n'))) {
          Dafny.ISequence<Dafny.Rune> _4_part;
          _4_part = (text).Subsequence(_1_start, _2_i);
          _0_segments = Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.Concat(_0_segments, Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.FromElements(_4_part));
          _1_start = (_2_i) + (BigInteger.One);
        }
        _2_i = (_2_i) + (BigInteger.One);
      }
      Dafny.ISequence<Dafny.Rune> _5_lastPart;
      _5_lastPart = (text).Subsequence(_1_start, new BigInteger((text).Count));
      _0_segments = Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.Concat(_0_segments, Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.FromElements(_5_lastPart));
      lines = _0_segments;
      return lines;
    }
    public static Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> SplitOnWhitespace(Dafny.ISequence<Dafny.Rune> text)
    {
      Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> tokens = Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.Empty;
      Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _0_parts;
      _0_parts = Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.FromElements();
      BigInteger _1_i;
      _1_i = BigInteger.Zero;
      while ((_1_i) < (new BigInteger((text).Count))) {
        while (((_1_i) < (new BigInteger((text).Count))) && (IptablesToSmt.__default.IsWhitespace((text).Select(_1_i)))) {
          _1_i = (_1_i) + (BigInteger.One);
        }
        ...
        if (((text).Select(_1_i)) == (new Dafny.Rune('"'))) {
          BigInteger _2_start;
          _2_start = (_1_i) + (BigInteger.One);
          _1_i = (_1_i) + (BigInteger.One);
          while (((_1_i) < (new BigInteger((text).Count))) && (((text).Select(_1_i)) != (new Dafny.Rune('"')))) {
            if ((((text).Select(_1_i)) == (new Dafny.Rune('\\'))) && (((_1_i) + (BigInteger.One)) < (new BigInteger((text).Count)))) {
              _1_i = (_1_i) + (new BigInteger(2));
            } else {
              _1_i = (_1_i) + (BigInteger.One);
            }
          }
          Dafny.ISequence<Dafny.Rune> _3_value;
          _3_value = (text).Subsequence(_2_start, _1_i);
          if ((new BigInteger((_3_value).Count)).Sign == 1) {
            _0_parts = Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.Concat(_0_parts, Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.FromElements(_3_value));
          }
          if (((_1_i) < (new BigInteger((text).Count))) && (((text).Select(_1_i)) == (new Dafny.Rune('"')))) {
            _1_i = (_1_i) + (BigInteger.One);
          }
        } else {
          BigInteger _4_start2;
          _4_start2 = _1_i;
          while (((_1_i) < (new BigInteger((text).Count))) && (!(IptablesToSmt.__default.IsWhitespace((text).Select(_1_i))))) {
            _1_i = (_1_i) + (BigInteger.One);
          }
          Dafny.ISequence<Dafny.Rune> _5_token;
          _5_token = (text).Subsequence(_4_start2, _1_i);
          if ((new BigInteger((_5_token).Count)).Sign == 1) {
    ...
    public static Dafny.ISequence<Dafny.Rune> Trim(Dafny.ISequence<Dafny.Rune> text)
    {
      Dafny.ISequence<Dafny.Rune> trimmed = Dafny.Sequence<Dafny.Rune>.Empty;
      if ((new BigInteger((text).Count)).Sign == 0) {
        trimmed = text;
        return trimmed;
      }
      BigInteger _0_start;
      _0_start = BigInteger.Zero;
      BigInteger _1_stop;
      _1_stop = new BigInteger((text).Count);
      while (((_0_start) < (_1_stop)) && (IptablesToSmt.__default.IsWhitespace((text).Select(_0_start)))) {
        _0_start = (_0_start) + (BigInteger.One);
      }
      while (((_1_stop) > (_0_start)) && (IptablesToSmt.__default.IsWhitespace((text).Select((_1_stop) - (BigInteger.One))))) {
        _1_stop = (_1_stop) - (BigInteger.One);
      }
      trimmed = (text).Subsequence(_0_start, _1_stop);
      return trimmed;
    }
    public static bool IsWhitespace(Dafny.Rune ch) {
      return ((((ch) == (new Dafny.Rune(' '))) || ((ch) == (new Dafny.Rune('\t')))) || ((ch) == (new Dafny.Rune('\r')))) || ((ch) == (new Dafny.Rune('\n')));
    }
  }

  public interface _ITarget {
    bool is_TargetAccept { get; }
    bool is_TargetDrop { get; }
    bool is_TargetReject { get; }
    bool is_TargetReturn { get; }
    bool is_TargetJump { get; }
    Dafny.ISequence<Dafny.Rune> dtor_name { get; }
    _ITarget DowncastClone();
  }
  public abstract class Target : _ITarget {
    public Target() {
    }
    private static readonly IptablesToSmt._ITarget theDefault = create_TargetAccept();
    public static IptablesToSmt._ITarget Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<IptablesToSmt._ITarget> _TYPE = new Dafny.TypeDescriptor<IptablesToSmt._ITarget>(IptablesToSmt.Target.Default());
    public static Dafny.TypeDescriptor<IptablesToSmt._ITarget> _TypeDescriptor() {
      return _TYPE;
    }
    public static _ITarget create_TargetAccept() {
      return new Target_TargetAccept();
    }
    public static _ITarget create_TargetDrop() {
      return new Target_TargetDrop();
    }
    public static _ITarget create_TargetReject() {
      return new Target_TargetReject();
    }
    public static _ITarget create_TargetReturn() {
      return new Target_TargetReturn();
    }
    public static _ITarget create_TargetJump(Dafny.ISequence<Dafny.Rune> name) {
      return new Target_TargetJump(name);
    }
    public bool is_TargetAccept { get { return this is Target_TargetAccept; } }
    public bool is_TargetDrop { get { return this is Target_TargetDrop; } }
    public bool is_TargetReject { get { return this is Target_TargetReject; } }
    public bool is_TargetReturn { get { return this is Target_TargetReturn; } }
    public bool is_TargetJump { get { return this is Target_TargetJump; } }
    public Dafny.ISequence<Dafny.Rune> dtor_name {
      get {
        var d = this;
        return ((Target_TargetJump)d)._name;
      }
    }
    public abstract _ITarget DowncastClone();
  }
  public class Target_TargetAccept : Target {
    public Target_TargetAccept() : base() {
    }
    public override _ITarget DowncastClone() {
      if (this is _ITarget dt) { return dt; }
      return new Target_TargetAccept();
    }
    public override bool Equals(object other) {
      var oth = other as IptablesToSmt.Target_TargetAccept;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
    ...
    public override string ToString() {
      string s = "IptablesToSmt.Target.TargetAccept";
      return s;
    }
  }
  public class Target_TargetDrop : Target {
    public Target_TargetDrop() : base() {
    }
    public override _ITarget DowncastClone() {
      if (this is _ITarget dt) { return dt; }
      return new Target_TargetDrop();
    ...
    public override bool Equals(object other) {
      var oth = other as IptablesToSmt.Target_TargetDrop;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
    ...
    public override string ToString() {
      string s = "IptablesToSmt.Target.TargetDrop";
      return s;
    }
  }
  public class Target_TargetReject : Target {
    public Target_TargetReject() : base() {
    }
    public override _ITarget DowncastClone() {
      if (this is _ITarget dt) { return dt; }
      return new Target_TargetReject();
    ...
    public override bool Equals(object other) {
      var oth = other as IptablesToSmt.Target_TargetReject;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
    ...
    public override string ToString() {
      string s = "IptablesToSmt.Target.TargetReject";
      return s;
    }
  }
  public class Target_TargetReturn : Target {
    public Target_TargetReturn() : base() {
    }
    public override _ITarget DowncastClone() {
      if (this is _ITarget dt) { return dt; }
      return new Target_TargetReturn();
    ...
    public override bool Equals(object other) {
      var oth = other as IptablesToSmt.Target_TargetReturn;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
    ...
    public override string ToString() {
      string s = "IptablesToSmt.Target.TargetReturn";
      return s;
    }
  }
  public class Target_TargetJump : Target {
    public readonly Dafny.ISequence<Dafny.Rune> _name;
    public Target_TargetJump(Dafny.ISequence<Dafny.Rune> name) : base() {
      this._name = name;
    }
    public override _ITarget DowncastClone() {
      if (this is _ITarget dt) { return dt; }
      return new Target_TargetJump(_name);
    }
    public override bool Equals(object other) {
      var oth = other as IptablesToSmt.Target_TargetJump;
    ...
    public override string ToString() {
      string s = "IptablesToSmt.Target.TargetJump";
      s += "(";
  ...
  public interface _IFieldMatch {
    bool is_MatchAny { get; }
    bool is_MatchExact { get; }
    Dafny.ISequence<Dafny.Rune> dtor_value { get; }
    _IFieldMatch DowncastClone();
  }
  public abstract class FieldMatch : _IFieldMatch {
    public FieldMatch() {
    }
    private static readonly IptablesToSmt._IFieldMatch theDefault = create_MatchAny();
    public static IptablesToSmt._IFieldMatch Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<IptablesToSmt._IFieldMatch> _TYPE = new Dafny.TypeDescriptor<IptablesToSmt._IFieldMatch>(IptablesToSmt.FieldMatch.Default());
    public static Dafny.TypeDescriptor<IptablesToSmt._IFieldMatch> _TypeDescriptor() {
      return _TYPE;
    }
    public static _IFieldMatch create_MatchAny() {
      return new FieldMatch_MatchAny();
    }
    public static _IFieldMatch create_MatchExact(Dafny.ISequence<Dafny.Rune> @value) {
      return new FieldMatch_MatchExact(@value);
    }
    public bool is_MatchAny { get { return this is FieldMatch_MatchAny; } }
    public bool is_MatchExact { get { return this is FieldMatch_MatchExact; } }
    public Dafny.ISequence<Dafny.Rune> dtor_value {
      get {
        var d = this;
        return ((FieldMatch_MatchExact)d)._value;
      }
    }
    public abstract _IFieldMatch DowncastClone();
  }
  public class FieldMatch_MatchAny : FieldMatch {
    public FieldMatch_MatchAny() : base() {
    }
    public override _IFieldMatch DowncastClone() {
      if (this is _IFieldMatch dt) { return dt; }
      return new FieldMatch_MatchAny();
    }
    public override bool Equals(object other) {
      var oth = other as IptablesToSmt.FieldMatch_MatchAny;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
    ...
    public override string ToString() {
      string s = "IptablesToSmt.FieldMatch.MatchAny";
      return s;
    }
  }
  public class FieldMatch_MatchExact : FieldMatch {
    public readonly Dafny.ISequence<Dafny.Rune> _value;
    public FieldMatch_MatchExact(Dafny.ISequence<Dafny.Rune> @value) : base() {
      this._value = @value;
    }
    public override _IFieldMatch DowncastClone() {
      if (this is _IFieldMatch dt) { return dt; }
      return new FieldMatch_MatchExact(_value);
    }
    public override bool Equals(object other) {
      var oth = other as IptablesToSmt.FieldMatch_MatchExact;
    ...
    public override string ToString() {
      string s = "IptablesToSmt.FieldMatch.MatchExact";
      s += "(";
  ...
  public interface _IRule {
    bool is_Rule { get; }
    Dafny.ISequence<Dafny.Rune> dtor_chain { get; }
    IptablesToSmt._IFieldMatch dtor_source { get; }
    IptablesToSmt._IFieldMatch dtor_destination { get; }
    IptablesToSmt._IFieldMatch dtor_protocol { get; }
    IptablesToSmt._IFieldMatch dtor_srcPort { get; }
    IptablesToSmt._IFieldMatch dtor_dstPort { get; }
    IptablesToSmt._ITarget dtor_target { get; }
    BigInteger dtor_lineNumber { get; }
    Dafny.ISequence<Dafny.Rune> dtor_original { get; }
    _IRule DowncastClone();
  }
  public class Rule : _IRule {
    public readonly Dafny.ISequence<Dafny.Rune> _chain;
    public readonly IptablesToSmt._IFieldMatch _source;
    public readonly IptablesToSmt._IFieldMatch _destination;
    public readonly IptablesToSmt._IFieldMatch _protocol;
    public readonly IptablesToSmt._IFieldMatch _srcPort;
    public readonly IptablesToSmt._IFieldMatch _dstPort;
    public readonly IptablesToSmt._ITarget _target;
    public readonly BigInteger _lineNumber;
    public readonly Dafny.ISequence<Dafny.Rune> _original;
    public Rule(Dafny.ISequence<Dafny.Rune> chain, IptablesToSmt._IFieldMatch source, IptablesToSmt._IFieldMatch destination, IptablesToSmt._IFieldMatch protocol, IptablesToSmt._IFieldMatch srcPort, IptablesToSmt._IFieldMatch dstPort, IptablesToSmt._ITarget target, BigInteger lineNumber, Dafny.ISequence<Dafny.Rune> original) {
      this._chain = chain;
      this._source = source;
    ...
    public _IRule DowncastClone() {
      if (this is _IRule dt) { return dt; }
      return new Rule(_chain, _source, _destination, _protocol, _srcPort, _dstPort, _target, _lineNumber, _original);
    }
    public override bool Equals(object other) {
      var oth = other as IptablesToSmt.Rule;
      return oth != null && object.Equals(this._chain, oth._chain) && object.Equals(this._source, oth._source) && object.Equals(this._destination, oth._destination) && object.Equals(this._protocol, oth._protocol) && object.Equals(this._srcPort, oth._srcPort) && object.Equals(this._dstPort, oth._dstPort) && object.Equals(this._target, oth._target) && this._lineNumber == oth._lineNumber && object.Equals(this._original, oth._original);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._chain));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._source));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._destination));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._protocol));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._srcPort));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._dstPort));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._target));
    ...
    public override string ToString() {
      string s = "IptablesToSmt.Rule.Rule";
      s += "(";
      s += this._chain.ToVerbatimString(true);
      s += ", ";
      s += Dafny.Helpers.ToString(this._source);
      s += ", ";
      s += Dafny.Helpers.ToString(this._destination);
      s += ", ";
      s += Dafny.Helpers.ToString(this._protocol);
      s += ", ";
      s += Dafny.Helpers.ToString(this._srcPort);
      s += ", ";
      s += Dafny.Helpers.ToString(this._dstPort);
      s += ", ";
      s += Dafny.Helpers.ToString(this._target);
      s += ", ";
      s += Dafny.Helpers.ToString(this._lineNumber);
      s += ", ";
      s += this._original.ToVerbatimString(true);
      s += ")";
      return s;
    }
    private static readonly IptablesToSmt._IRule theDefault = create(Dafny.Sequence<Dafny.Rune>.Empty, IptablesToSmt.FieldMatch.Default(), IptablesToSmt.FieldMatch.Default(), IptablesToSmt.FieldMatch.Default(), IptablesToSmt.FieldMatch.Default(), IptablesToSmt.FieldMatch.Default(), IptablesToSmt.Target.Default(), BigInteger.Zero, Dafny.Sequence<Dafny.Rune>.Empty);
    public static IptablesToSmt._IRule Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<IptablesToSmt._IRule> _TYPE = new Dafny.TypeDescriptor<IptablesToSmt._IRule>(IptablesToSmt.Rule.Default());
    public static Dafny.TypeDescriptor<IptablesToSmt._IRule> _TypeDescriptor() {
      return _TYPE;
    }
    public static _IRule create(Dafny.ISequence<Dafny.Rune> chain, IptablesToSmt._IFieldMatch source, IptablesToSmt._IFieldMatch destination, IptablesToSmt._IFieldMatch protocol, IptablesToSmt._IFieldMatch srcPort, IptablesToSmt._IFieldMatch dstPort, IptablesToSmt._ITarget target, BigInteger lineNumber, Dafny.ISequence<Dafny.Rune> original) {
      return new Rule(chain, source, destination, protocol, srcPort, dstPort, target, lineNumber, original);
    }
    public static _IRule create_Rule(Dafny.ISequence<Dafny.Rune> chain, IptablesToSmt._IFieldMatch source, IptablesToSmt._IFieldMatch destination, IptablesToSmt._IFieldMatch protocol, IptablesToSmt._IFieldMatch srcPort, IptablesToSmt._IFieldMatch dstPort, IptablesToSmt._ITarget target, BigInteger lineNumber, Dafny.ISequence<Dafny.Rune> original) {
      return create(chain, source, destination, protocol, srcPort, dstPort, target, lineNumber, original);
    }
    public bool is_Rule { get { return true; } }
    public Dafny.ISequence<Dafny.Rune> dtor_chain {
      get {
        return this._chain;
      }
    }
    public IptablesToSmt._IFieldMatch dtor_source {
      get {
        return this._source;
      }
    }
    public IptablesToSmt._IFieldMatch dtor_destination {
      get {
        return this._destination;
      }
    }
    public IptablesToSmt._IFieldMatch dtor_protocol {
      get {
        return this._protocol;
      }
    }
    public IptablesToSmt._IFieldMatch dtor_srcPort {
      get {
        return this._srcPort;
      }
    }
    public IptablesToSmt._IFieldMatch dtor_dstPort {
      get {
        return this._dstPort;
      }
    }
    public IptablesToSmt._ITarget dtor_target {
      get {
        return this._target;
      }
    }
    ...
    public Dafny.ISequence<Dafny.Rune> dtor_original {
      get {
...
class __CallToMain {
  public static void Main(string[] args) {
    Dafny.Helpers.WithHaltHandling(() => IptablesToSmt.__default._Main(Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.UnicodeFromMainArguments(args)));
  }
}
